    ...
    with tab1:
        ...
        with col2:
            if st.button("🔄 초기화", use_container_width=True):
                default_guides = {
                    "사주": {"목차": ["총운", "성격분석", "재물운", "직업운", "건강운", "연애운", "년도별운세"], "지침": ""},
                    "연애": {"목차": ["연애총운", "이상형분석", "연애스타일", "궁합분석", "미래전망"], "지침": ""},
                    "타로": {"목차": ["현재상황", "장애물", "조언", "미래전망", "종합해석"], "지침": ""}
                }
                st.session_state.settings["guides"][selected_service] = default_guides[selected_service]
                save_settings(st.session_state.settings)
                st.rerun()
